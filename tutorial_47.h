#ifndef TUTORIAL_47_H_INCLUDED
#define TUTORIAL_47_H_INCLUDED
...
/*
    Tutorial 47: Mutexes and Conditions
*/
...
bool init(){
    ...
    if( SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) < 0){
		...
		srand( SDL_GetTicks() );
        //Crear la ventana
        gWindow = SDL_CreateWindow("Tutorial 47", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if( gWindow == NULL){
...
void produce()
{
    ...
    if(gData != -1)
    {
        //Wait buffer is cleared
        std::cout << "\nProducer encounter full buffer, waiting for consumer to empty buffer.." << std::endl;
