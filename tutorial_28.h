#ifndef TUTORIAL_28_H_INCLUDED
#define TUTORIAL_28_H_INCLUDED
...
bool init(){
    ...
    if( SDL_Init(SDL_INIT_VIDEO) < 0){
        ...
        //Crear la ventana
        gWindow = SDL_CreateWindow("Tutorial 28", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if( gWindow == NULL){
...
bool loadMedia(){
    ...
    {
        std::cout << "Error al cargar la imagen jpg" << std::endl;
        success = false;
    }
    ...
    if(!init()){
        ...
        if(!loadMedia()){
            ...
            while(!quit)
            {
                ...
                camera.y = (dot.getPoxY() + Dot::DOT_HEIGHT / 2) - SCREEN_HEIGHT / 2;
