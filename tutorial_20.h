#ifndef TUTORIAL_20_H_INCLUDED
#define TUTORIAL_20_H_INCLUDED
...
bool init(){
    ...
    if( SDL_Init(SDL_INIT_VIDEO) < 0){
        ...
        //Crear la ventana
        gWindow = SDL_CreateWindow("Tutorial 20", 0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        if( gWindow == NULL){
...
bool loadMedia(){
    ...
    if(gFont == NULL){
        ...
        if(!gPromptTexture.loadFromRenderedText("Este texto es de ejemplo de texto renderizado", textColor)){
        ...
        if(!loadMedia()){
            ...
            while(!quit)
            {
                while(SDL_PollEvent(&eventHandler) != 0)
                {
                    ...
                    else if(eventHandler.type == SDL_KEYDOWN && eventHandler.key.keysym.sym == SDLK_RETURN)
                    {
